...
class TcpSocket(object):
     ...
     def connect(self, hostname, dest_port = 80):
     	...
     	self.src_ip = get_local_host()
     	self.dest_ip = socket.gethostbyname(hostname)
     	self.src_port = get_port() # here we can randomly pick one, but I'm not sure whether it is correct
     	self.data = ''
     ...
     def send(self, data):
        ...
        build a new tcp packet and send it to the dest ip, then wait for the ack.
     	If the wait time exceeds the timeout, retransmit this packet slow start
        """
     	self.cwnd = self.MSS
     	while len(data) > 0:
