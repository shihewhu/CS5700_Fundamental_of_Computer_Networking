...
tcpdump -tnr $file  | awk -F '.' '{print $1"."$2"."$3"."$4}' | sort | uniq | awk -F ' ' '{print $2}' | sort | uniq | while read line; 
do echo "$line	$(whois $line | grep OrgName | sed 's/   */	/g' | cut -f2)"; done >> ip_to_identity.txt
